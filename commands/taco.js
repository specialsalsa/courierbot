...
module.exports = {
    ...
    async execute(message, args) {
        const checkIngredients = async (message, user) => {
            ...
            if (!currentIngredients) {
                message.channel.send(`You don't have any ingredients yet! Keep posting to score some ingredients.`);
                return;
            }
            let secondArray = Object.entries(currentIngredients);

            secondArray = secondArray.filter(([ingredient, quantity]) => quantity >= 1 && ingredient !== 'messageCount');
            ...
            if (!args[1]) {
            message.channel.send(`Your current ingredient collection: \n\n ${secondArrayFormatted}`);
            } else {
                message.channel.send(`${user.username}'s ingredient collection: \n \n ${secondArrayFormatted}`);
            }
        ...
        if (args[0] == 'check') {
            ...
            if (args[1] == 'help') {
                message.channel.send(`Usage: Trade an ingredient you have for an ingredient from another member with format: \`.taco trade @user yourIngredient for theirIngredient\``);
                return;
            }
            ...
            let firstFoundIngredient = traderArray.find(([ingredient, quantity]) => ingredient == traderIngredient && quantity >= 1);

            if (!firstFoundIngredient) {
                message.channel.send(`${message.member.displayName}, you don't have any of that ingredient to trade!`);
                return;
            }
            
            let tradeeArray = Object.entries(tradeeMemberIngredients);

            let secondFoundIngredient = tradeeArray.find(([ingredient, quantity]) => ingredient == tradeeIngredient && quantity >= 1);

            if (!secondFoundIngredient) {
                message.channel.send(`${message.member.displayName}, the person you are trying to trade with does not have any of your desired ingredient.`);
                return;
            }

            message.channel.send(`${tradeeMember}, ${message.member.displayName} wants to trade their ${traderIngredient} for your ${tradeeIngredient}. Do you accept?
            ...
            const collectedMessage = await message.channel.awaitMessages(filter, {time: 120000, max: 1, errors: ['time']});

            response = collectedMessage.map(cm => cm.content).filter(cont => cont.toLowerCase() == 'yes' || cont.toLowerCase() == 'no').toString();
            ...
            if (response.toLowerCase().includes('yes')) {
                ...
                message.channel.send(`${message.member.displayName}, ${tradeeMember.username} has completed the trade! You swapped one of your ${traderIngredient} for one of their ${tradeeIngredient}.`);
        
            } else if (response.toLowerCase().includes('no')) {
                message.channel.send(`${message.member.displayName}, ${tradeeMember.username} has declined the trade.`);
            } else {
                message.channel.send(`Unable to complete the trade`);
            }

        } catch (e) {
            message.channel.send(`${message.member.displayName}, ${tradeeMember} took too long to respond.`);
            console.log(e);
        };
        ...
        message.channel.send(`List of members with ${ingredient}: \n \n ${formattedString}`);


    } else if (args[0] == 'shop') {

        let memberIngredients = await cb.tacos.get(message.member.id);
        message.channel.send(`Welcome to the CourierBot Taco Shop! Here, you can trade your excess ingredients for any ingredient available in the shop up to 3 times today.
        ...
        if (memberIngredients.shopCount == 3) { message.channel.send(`Sorry, you have used the shop 3 times already. Trade with other members to get your desired ingredients!`);
        return;
        }
        ...
        if (!response.some(ingredient => Object.keys(cb.tacoIngredients).includes(ingredient))) {
            ...
            Object.keys(cb.tacoIngredients).forEach(async ingredient => {
                if (thisIngredient == ingredient) {
                    ...
                    if (!memberIngredients[desiredIngredient]) memberIngredients[desiredIngredient] = 1;
        ...
        message.channel.send(`You swapped one of your ${thisIngredient} for a ${desiredIngredient} at the shop.`);
    ...
    const assembledTacos = { 
        ...
        'SUPREME TACO': 
            { 
                shell: 1, beef: 1, cheese: 1, lettuce: 1, tomatoes: 1
            }, 
        'AZALEA\'S TACO': 
            { 
                shell: 1, beef: 1, cheese: 1, 'hot sauce': 1
            }, 
        'GUACAMOLE TACO': 
            { 
                shell: 1, beef: 1, cheese: 1, guacamole: 1, lettuce: 1
            }, 
        'BEEF AND BEAN TACO': 
            { 
                shell: 1, beef: 1, cheese: 1, beans: 1
            }
        };
    ...
    if (!madeTaco) {
        message.channel.send(`Sorry, you don't have enough ingredients to assemble any tacos at the moment.`);
    }
