...
// };
        ...
        if (!args[0]) {
        try {
        await cb.endb.set(`${message.author.tag}`, JSON.stringify(
            {
        ...
        message.channel.send(`Successfully set database entry for ${JSON.parse(await cb.endb.get("specialsal")).name}`)
    } catch (e) {
        console.log(e);
    }
    } else if (args[0] == "check") {
        message.channel.send(JSON.parse(await cb.endb.get("specialsal")).name)
    }
}
