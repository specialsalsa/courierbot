  ...
  async execute(message, args) {
    ...
    const winBattle = async function (whoWon) {
      ...
      if (!loser.eggs || loser.eggs <= 0) {
        message.channel.send(
          `${loser.username} lost, but they don't have any eggs for ${winner.username} to steal. No egg counts were affected.`
        );
        return;
      }
      ...
      message.channel.send(
        `${winner.username}, you won the battle! You stole an egg from ${loser.username}!`
      );
      message.channel.send(
        `${winner.username} now has ${winner.eggs} eggs, and ${loser.username} now has ${loser.eggs} eggs.`
      );
    };

    const tie = () => {
      message.channel.send(
        `${player1.username}, Tie! Neither egg count was affected.`
      );
    };
    ...
    if (args[0] == 'give') {
      ...
      if (message.member.id === mentionedUser.id) {
        message.channel.send(
          'Love the self-love, but you cannot give an egg to yourself. Sorry.'
        );
        return;
      }
      ...
      if (!eggs) {
        ...
        message.channel.send(
          `Aw, how nice! ${message.member.displayName} gave an egg to ${mentionedUser.username}!`
        );
        message.channel.send(
          `${message.member.displayName} now has ${eggs} eggs, and ${mentionedUser.username} now has ${eggs2} eggs.`
        );
      }
    }
    ...
    if (args[0] == 'youllneverguessthis') {
      ...
      if (message.mentions.users.first() == message.author) {
        message.channel.send(
          `You cannot battle yourself! Please specify another member.`
        );
        return;
      }

      message.channel.send(
        `${mentionedUser.username}, ${message.member.displayName} has challenged you to an Egg Battle! ${message.author} makes the first throw (post rock, paper, or scissors)`
      );
      ...
      try {
        const collectedMessageFirst = await message.channel.awaitMessages(
          filter1,
          { time: 60000, max: 1, errors: ['time'] }
        );

        responseFirst = collectedMessageFirst
          .map(cm => cm.content)
          .filter(
            cont =>
              cont.toLowerCase().includes('rock') ||
              cont.toLowerCase().includes('paper') ||
              cont.toLowerCase().includes('scissors')
          )
          .toString();
        ...
        console.log(e);
        message.channel.send(
          `${message.member.displayName} didn't throw in time. No egg counts were affected.`
        );
      }

      message.channel.send(
        `${mentionedUser}, ${message.member.displayName} has made their throw. Make your throw to defend your eggs! (post rock, paper, or scissors)`
      );
      ...
      try {
        const collectedMessageSecond = await message.channel.awaitMessages(
          filter,
          { time: 60000, max: 1, errors: ['time'] }
        );

        let responseSecond = collectedMessageSecond
          .map(cm => cm.content)
          .filter(
            cont =>
              cont.toLowerCase().includes('rock') ||
        ...
        console.log(e);
        message.channel.send(
          `${mentionedUser.username} took too long to reply! No egg counts were affected.`
        );
      }
    }
  }
